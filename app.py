...
st.write(
    "Gib einen Freitext oder Artikel ein, um die "
    "Kategorie und Modell-Konfidenz zu bestimmen."
)

threshold = st.sidebar.slider(
    "Uncertainty Threshold", min_value=0.20, max_value=0.80, value=0.45, step=0.05
)
predictor.threshold = threshold

text_input = st.text_area(
    "Artikeltext oder Überschrift:",
    height=150,
    placeholder=(
        "E.g., Solar and wind energy capacities grew significantly this year..."
    ),
)
...
if st.button("Klassifizieren"):
    if text_input.strip():
        ...
        if result["category"] == "Uncertain / No Clear Focus":
            cat = result["category"]
            st.warning(f"Kategorie: **{cat}** (Sicherheit unter {threshold:.0%})")
        else:
            cat = result["category"]
            conf = result["confidence"]
            st.success(f"Kategorie: **{cat}** (Konfidenz: {conf:.2%})")
