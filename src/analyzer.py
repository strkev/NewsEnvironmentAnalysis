...
def analyze_top_features(model, tfidf_vectorizer, top_n=10):
    ...
    if hasattr(model, "calibrated_classifiers_"):
        coefs = np.mean(
            [clf.estimator.coef_ for clf in model.calibrated_classifiers_], axis=0
        )
...
def analyze_predictions_and_errors(
    model, tfidf_vectorizer, X_test_raw, y_test, num_samples=3
):
    ...
    for idx, row in errors.head(num_samples).iterrows():
        t_label, p_label, conf = (
            row["true_label"],
            row["predicted_label"],
            row["confidence"],
        )
        snippet = row["text"][:120]
        print(f"True: {t_label} | Pred: {p_label} ({conf:.2%}) -> {snippet}...")
